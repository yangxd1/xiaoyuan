<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","margin":"0 auto","color":"#666","borderRadius":"0 0 10px 10px","background":"#fff","width":"calc(100% - 60px)","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 10px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"100%","margin":"10px 0 10PX","color":"rgba(19,161,230,1)","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('shetuanxinxi','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"border":"1px solid rgba(19,161,230,.2)","width":"100%","padding":"0","borderRadius":"0px","background":"#fff"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="jiantuanshijian"
						label="建团时间">
						<template slot-scope="scope">
							{{scope.row.jiantuanshijian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="rutuanzhuangtai"
						label="入团状态">
						<template slot-scope="scope">
							{{scope.row.rutuanzhuangtai}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('shetuanxinxi','加入申请')" @click="shetuanjiaruCrossAddOrUpdateHandler(scope.row,'cross','是','rutuanzhuangtai','已加入','已加入,未加入'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								加入申请
							</el-button>
							<el-button class="btn8" v-if="isAuth('shetuanxinxi','申请退出')" @click="tuichushetuanCrossAddOrUpdateHandler(scope.row,'cross','是','[1]','已退出')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								申请退出
							</el-button>
							<el-button class="edit" v-if=" isAuth('shetuanxinxi','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai10" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								改动
							</el-button>

							<el-button class="view" v-if="isAuth('shetuanxinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看评论
							</el-button>
		...
		<shetuanjiaru-cross-add-or-update v-if="shetuanjiaruCrossAddOrUpdateFlag" :parent="this" ref="shetuanjiaruCrossaddOrUpdate"></shetuanjiaru-cross-add-or-update>
		<tuichushetuan-cross-add-or-update v-if="tuichushetuanCrossAddOrUpdateFlag" :parent="this" ref="tuichushetuanCrossaddOrUpdate"></tuichushetuan-cross-add-or-update>
...
import shetuanjiaruCrossAddOrUpdate from "../shetuanjiaru/add-or-update";
import tuichushetuanCrossAddOrUpdate from "../tuichushetuan/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				shetuanjiaruCrossAddOrUpdateFlag: false,
				tuichushetuanCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			shetuanjiaruCrossAddOrUpdate,
			tuichushetuanCrossAddOrUpdate,
		},
		methods: {
			...
			shetuanjiaruCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				this.shetuanjiaruCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.shetuanjiaruCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.shetuanjiaruCrossaddOrUpdate.init(row.id,type);
				});
			},
			tuichushetuanCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				if(crossOptAudit=='是'&&row.sfsh!='是') {
					this.$message({
						message: "请审核通过后再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.tuichushetuanCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','shetuanxinxi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.tuichushetuanCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.tuichushetuanCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    shBatchHandler(){
      this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
        ...
        this.$http({
          url: "shetuanxinxi/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
          method: "post",
          data: this.batchIds
        }).then(({ data }) => {
          if (data && data.code === 0) {

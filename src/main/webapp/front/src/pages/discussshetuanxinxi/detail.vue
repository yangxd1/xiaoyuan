...
<div>
	<div :style='{"width":"1200px","padding":"20px","margin":"0px auto","borderRadius":"27px","background":"#fff"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'Ξ'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/discussshetuanxinxi"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"width":"50%","padding":"0 20px","position":"relative","display":"flex","order":"1"}'>
			...
			<div class="info" :style='{"padding":"10px","margin":"0","background":"#fff","flex":"1"}'>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"5px","outline":"none","color":"#fff","borderRadius":"4px","background":"#005FBA","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('discussshetuanxinxi','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"1px solid #343434","cursor":"pointer","padding":"0 10px","margin":"5px","outline":"none","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('discussshetuanxinxi','删除')" @click="delClick">删除</el-button>
					<!-- hasPay $hasPay -->
				</div>
			</div>
		</div>
    ...
    methods: {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('discussdiscussshetuanxinxi/delete',[id]).then(res=>{
	  ...
	  addDiscussNum(type){
		  ...
		  this.$http.post('discussshetuanxinxi/update',this.detail).then(res=>{
		...
		// 修改
		editClick(){
			this.$router.push(`/index/discussshetuanxinxiAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此社团信息评论表？')
			  .then(_ => {
			    this.$http.post('discussshetuanxinxi/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
